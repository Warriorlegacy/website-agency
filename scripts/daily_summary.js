...
import { sendTelegramDocument } from './lib/telegram_notifier.js';
...
<i>Next automated cycle scheduled in 2 hours. Full report document attached below.</i>`;
  ...
  // Compile and attach complete workflow run report as a markdown document file
  try {
    const docFilename = `workflow_run_${stats.date}_${Date.now()}.md`;
...
`;
    ...
    await sendTelegramDocument(docFilename, docContent, `📄 <b>Attached:</b> Complete Workflow Run Audit Document (<code>${docFilename}</code>)`);
    console.log(`✅ Workflow run report document (${docFilename}) delivered to Telegram!`);
  } catch (err) {
